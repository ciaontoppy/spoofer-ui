...
bool items::button(const char* name, ImVec2 pos, ImVec2 size, std::string id_)
{
	...
	ImVec4 bg_col = active ? colors::item_col2 : hovered ? colors::item_col : colors::item_col2;
	ImVec4 bg_col2 = active ? colors::item_col2 : hovered ? colors::item_col : colors::item_col2;
...
bool r_button(const char* name, ImVec2 pos, ImVec2 size = { 430, 46 })
{
	...
	ImVec4 r_col = r_act ? ImColor(48, 106, 194, 255) : r_hov ? ImColor(48, 106, 194, 255) : ImColor(64, 130, 231, 255);
...
bool items::button_hint(const char* name, ImVec2 pos, const char* fst_line, const char* scn_line)
{
	auto* window = ImGui::GetCurrentWindow();
	if (window->SkipItems) return false;
	...
	if (w.opened)
	{
		...
		ImGui::BeginChild("POPUP", window_size, false, Window_Flags);
		{
			...
			wi->DrawList->AddText(wi->Pos + ImVec2((wi->Size.x - ImGui::CalcTextSize(name).x) / 2, 30), ImGui::GetColorU32(colors::white), name);

			ImGui::PushFont(fonts::inter3);
			{

				wi->DrawList->AddText(wi->Pos + ImVec2(20, 100), ImGui::GetColorU32(colors::gray), fst_line);

				wi->DrawList->AddText(wi->Pos + ImVec2(20, 150), ImGui::GetColorU32(colors::red), scn_line);
			...
			float alpha = ((sin(time * 4.0f) * 0.5f + 0.5f) * (255 - 100) + 100) / 255.0f;

			ImVec4 col = ImVec4(0.321, 0.36, 0.545, alpha);

			ImGui::GetForegroundDrawList()->AddShadowRect(wi->Pos, wi->Pos + wi->Size, ImGui::GetColorU32(col), 50, { 0,0 }, ImDrawFlags_ShadowCutOutShapeBackground, _rounding);
			ImGui::GetForegroundDrawList()->AddRect(wi->Pos, wi->Pos + wi->Size, ImGui::GetColorU32(col), _rounding);
		}
		ImGui::End();
