...
void add_borders(ImVec2 size)
{
    ...
    const ImVec4 color = ImColor(72, 64, 163, 255);
...
void title(const char* name, const char* version, float _py)
{
    ...
    ImGui::ShadowText(name, ImGui::GetColorU32(colors::white), ImGui::GetColorU32(t), 100, ImVec2((window->Size.x - t_name.x) / 2 - t_name.x - 5, _py));
    ImGui::ShadowText(version, ImGui::GetColorU32(colors::main_color), ImGui::GetColorU32(colors::main_color), 100, ImVec2((window->Size.x - t_version.x - 35) / 2, _py));
    ImGui::ShadowText("LOGIN", ImGui::GetColorU32(colors::white), ImGui::GetColorU32(t), 100, ImVec2((window->Size.x - t_login.x) / 2 + t_login.x - 45, _py));
}
...
void custom::begin_child(const char* name, ImVec2 pos, ImVec2 size)
{
    ...
    ImGui::PushClipRect(pos - ImVec2(10, 10), pos + size + ImVec2(10, 10), false);
    window->DrawList->AddShadowRect(pos, pos + size, ImGui::GetColorU32(col), 130, { 0,0 }, ImDrawFlags_ShadowCutOutShapeBackground);
    ImGui::PopClipRect();
...
bool custom::announcement(const char* title, const char* description, ImVec2 pos)
{
    ...
    const ImVec4 description_col = ImColor(210, 187, 73, 255);
